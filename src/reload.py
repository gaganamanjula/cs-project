...
from database.userchats import get_phone_number_by_id


def reaload_function(credit_number, bot, message, user_id):
  try:
    start = time.time()
    rem = credit_number
    ...
    phone = get_phone_number_by_id(user_id)
    payload_paymentInit = {
        'paymentNumber': phone,
        'paymentAmount': '100',
        'blackout': 'false',
    ...
    payload_auth = {
        ...
        "ccaClientSessionId":
        "1_d599089c-0693-4043-8a35-ca76389b4180",  # "0_3906e45c-3f78-4bd1-aee5-4756e8e53c4f
        "ccaTiming": "7252",
    ...
    if result_auth["result"] == "Authenticate":
      ...
      formatted_result = f"RELOAD FAILED CHECK YOUR CREDIT CARD!"
      bot.edit_message_text(chat_id=message.chat.id,
                            message_id=message.message_id,
    ...
    if message_input and message_decision:
      ...
      formatted_result = (
          f"<b>[ϟ] RELOAD {message_decision_value} {symbol}</b>\n\n"
          f"<b>[ϟ] CARD :</b><code> {credit_number}</code>\n"
          f"<b>[ϟ] RESPONSE :</b> {message_value}\n"
          f"<b>[ϟ] REFERENCE :</b> {reference_number}\n"
          f"<b>[ϟ] TIME :</b> {signed_date_time}\n"
